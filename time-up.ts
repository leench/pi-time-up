...
export const DEFAULT_PROMPTS: Record<PromptAction, string> = {
	...
	"wrap-up": "Time-up soft deadline for {{label}}: the cutoff is {{cutoff}} (local time), with {{remaining}} remaining. This is a soft deadline; do not hard-kill the current tool.\n\nSwitch to wrap-up mode:\n- Do not start new large tasks.\n- Check the currently running subagents with `subagent({ action: \"status\" })`.\n- For each subagent still running, use `subagent({ action: \"steer\", id: \"<run-id>\", message: \"Finish the current tool call, then converge.\" })` and ask it to finish its current tool call, then converge.\n- Prioritize saving changes, essential verification, and a concise handoff.\n- Do not expand the scope.",
	"force-wrap-up": "Only {{remaining}} remain before the {{label}} cutoff at {{cutoff}}. Force wrap-up now: do not start or continue non-essential work. Check running children with `subagent({ action: \"status\" })`, then steer each one with `subagent({ action: \"steer\", id: \"<run-id>\", message: \"Finish the current tool call and converge immediately.\" })`. Save changes, perform only essential verification, and produce the final handoff. Do not hard-kill the current tool.",
